...
easywsclient::WebSocket::pointer from_url(const std::string& url, bool useMask, const std::string& origin, const std::string& extensions, const std::string& protocol) {
    char host[128];
    int port;
    char path[128];
    if (url.size() >= 128) {
    ...
    }
    {
        ...
        /* by undwad begin */
        if (!extensions.empty()) {
            snprintf(line, 256, "Sec-WebSocket-Extensions: %s\r\n", extensions.c_str()); ::send(sockfd, line, strlen(line), 0);
        }
        if (!protocol.empty()) {
...
WebSocket::pointer WebSocket::from_url(const std::string& url, const std::string& origin, const std::string& extensions, const std::string& protocol) {
    return ::from_url(url, true, origin, extensions, protocol);
}

WebSocket::pointer WebSocket::from_url_no_mask(const std::string& url, const std::string& origin, const std::string& extensions, const std::string& protocol) {
    return ::from_url(url, false, origin, extensions, protocol);
}
